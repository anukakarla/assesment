 ...
 */
export const generateColumns = (rowData: any[], addRowIconsList: ColDef[] = [], columnNames: ColumnConfigI): ColDef[] => {
            ...
            columnNames.activeColumnNames.map((key, index) => {
                if (object.hasOwnProperty(key.field)) {
                    const mappedColumn: ColDef = {
                        ...
                        field: key.field,
                        sortable: columnNames.enableSorting.findIndex(k => k === key.field) !== -1 ? true : false,
                        filter: columnNames.enableFiltering.findIndex(k => k === key.field) !== -1 ? true : false,
                        flex: key.flex === 0 ? 0 : 1,
                        width: key.width,
    ...
    columnDefinitions = columnDefinitions.filter(
        (column, index, self) =>
            index === self.findIndex((colAtIndex) => colAtIndex.field === column.field)
    );

    if (addRowIconsList.length > 0) {
        addRowIconsList.forEach((iconItem) => {
            const rowIndex = columnDefinitions.findIndex((ele) => ele.headerName?.toLowerCase() === iconItem.headerName?.toLowerCase());
